...
> =>
  Effect.gen(function* () {
    ...
    return {
      ...
      // v1 agents that only advertise `modes` (gemini-cli) take
      // `session/set_mode`, which ACP v2 removed. Agents that advertise both
      // but only handle `session/set_mode` (goose's handler for the config
      // option is unverified) fall back to it when the config option method
      // is missing or rejects its params.
      setMode: (modeId) =>
        Effect.gen(function* () {
          ...
          const setSessionMode = Effect.gen(function* () {
            const started = yield* getStartedState;
          ...
          // the mode it runs in, even when it ignored the requested one.
