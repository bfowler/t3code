...
Other settings the agent offers, such as reasoning effort, appear in the composer's model options
menu like they do for built-in providers. An approval mode setting appears only for agents whose
mode does not follow the thread's permission mode. Agents with their own plan and
...
approval rules, as Claude and Codex do. The thread's [permission mode](./permission-modes.md)
selects the matching mode in Codex (codex-acp), Claude (claude-acp), Gemini, Qwen Code, Goose, and
Mistral Vibe, so those agents hide their own mode picker. If one of them refuses the matching mode,
for example in a folder it does not trust, a thread in a stricter mode than **Full access** does
not start. Choose another permission mode or update the agent.

Other registry agents start in their own default mode and keep their mode picker. How far they are
confined depends on the agent's own settings, so use its sandbox and permission controls when that
matters.

When any registry agent asks for approval, T3 Code answers by the thread's permission mode: **Full
access** approves automatically, and **Supervised** shows the request in the conversation. File
reads and searches never wait for approval.
